...
export function AnovaTable({ result }: { result: AovResult }) {
  return (
    <>
      <div className="row" style={{ marginBottom: 12 }}>
        <span className="chip">Grand mean {result.grandMean.toFixed(3)}</span>
        {Number.isFinite(result.cv) && <span className="chip">CV {result.cv.toFixed(2)}%</span>}
        {result.lsd != null && (
          <span className="chip">
        ...
        <tbody>
          {result.anova.map((r) => (
            <tr key={r.source}>
              ...
              <td className="num">{r.ss != null ? r.ss.toFixed(3) : ''}</td>
              <td className="num">{r.ms != null ? r.ms.toFixed(3) : ''}</td>
