...
// The pending-write key for a single cell: `${headerId}:${plotId}:${subsample}`.
function cellKey(v: Pick<MeasurementValue, 'measurementHeaderId' | 'plotId' | 'subsample'>): string {
  return `${v.measurementHeaderId}:${v.plotId}:${v.subsample ?? 1}`
}
...
interface DataEntryViewProps {
  ...
  onValuesChange: (values: MeasurementValue[]) => void
  onEditColumns: () => void
  onAnalyze: () => void
}
...
export function DataEntryView({
  ...
  const excludedPlotIds = useMemo(
    () => new Set(plots.filter((p) => p.excluded).map((p) => p.id)),
    [plots]
  )
  const treatmentName = useMemo(() => {
    const m = new Map(treatments.map((t) => [t.id!, t]))
  ...
  const valueMap = useMemo(() => {
    ...
    for (const v of values) m.set(cellKey(v), v.value)
    return m
  }, [values])
  ...
  // derive.ts only reads these fields off a ProjectSnapshot; the rest of the shape is unused here.
  const deriveSnapshot = useMemo(
    () =>
      ({
    ...
    [headers, values, orderedPlots, treatments]
  )
  ...
  const calcValues = useMemo(() => {
    ...
    for (const h of headers) if (isCalculated(h)) m.set(h.id!, measurementPlotValues(deriveSnapshot, h))
    return m
  }, [deriveSnapshot, headers])
  ...
  }, [orderedPlots, headers, valueMap, treatmentName, maxSub, expanded])
  ...
  const onChange = (next: GridRow[]): void => {
    // Compute changed cells, apply them optimistically, and debounce the actual writes.
    ...
    const map = new Map(values.map((v) => [cellKey(v), v]))
 ...
 * via `measurements.saveMetadata`, which returns the updated header.
 */
function MeasurementMetadataPanel({
  ...
  const save = (h: MeasurementHeader, patch: Partial<MeasurementHeader>): void => {
    api.measurements
      ...
      .catch((e) => alert(e instanceof Error ? e.message : 'Failed to save measurement details'))
  }
