...
export function timingLabel(
  a: Pick<MeasurementDef, 'applicationRef' | 'daysAfter' | 'timing'>
): string {
...
/**
 * The measurement's calendar date, derived from the anchored application's actual date + the offset.
 * Returns '' when unanchored or the application has no recorded actual date.
 */
export function measurementDate(
  a: Pick<MeasurementDef, 'applicationRef' | 'daysAfter'>,
